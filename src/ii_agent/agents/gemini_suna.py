"""
GeminiSuna Agent
Adapted from Suna's implementation to work with ii-agent's system using XML-based function calling
"""
...
from ii_agent.tools.xml_tool_parser import XMLToolParser, parse_xml_tool_calls
from ii_agent.db.manager import DatabaseManager
from ii_agent.tools import AgentToolManager
from ii_agent.utils.constants import COMPLETE_MESSAGE
from ii_agent.utils.workspace_manager import WorkspaceManager
from ii_agent.prompts.gemini_suna_system_prompt import get_gemini_suna_system_prompt
...
class GeminiSuna(BaseAgent):
    name = "gemini_suna_agent"
    description = """
A Gemini-powered agent that uses XML-based tool calling format adapted from Suna's implementation.

This agent combines the power of Google's Gemini models with Suna's XML tool calling approach,
providing a robust alternative to traditional function calling for complex task execution.

Features:
- XML-based tool calling for better control and debugging
- Autonomous workflow management with todo.md files
- Advanced error handling and recovery
- Support for streaming responses
- Compatible with all ii-agent tools
"""
    ...
    def __init__(
        ...
        """Initialize the GeminiSuna agent.

        Args:
            client: The Gemini LLM client to use
        ...
        self.system_prompt = get_gemini_suna_system_prompt(tools)
    ...
    def _parse_xml_tool_calls_from_response(self, text_response: str) -> List[Dict[str, Any]]:
        """Parse XML tool calls from the text response."""
        xml_tool_calls = parse_xml_tool_calls(text_response, strict_mode=False)
        
        # Convert XMLToolCall objects to our tool call format
        tool_calls = []
        for xml_call in xml_tool_calls:
            tool_call = {
                "tool_call_id": f"xml_call_{uuid.uuid4().hex[:8]}",
                "tool_name": xml_call.function_name,
                "tool_input": xml_call.parameters
            }
            tool_calls.append(tool_call)
            
        return tool_calls

    def _create_tool_call_parameters(self, tool_call: Dict[str, Any]) -> ToolCallParameters:
        """Create ToolCallParameters from a tool call dict."""
    ...
    def run_impl(
        ...
        while remaining_turns > 0:
            ...
            # Get model response WITHOUT tool parameters (since we use XML parsing)
            model_response, _ = self.client.generate(
                messages=self.history.get_messages_for_llm(),
                max_tokens=self.max_output_tokens,
                tools=[],  # No tools for Gemini - we parse XML instead
            ...
            # Extract text response to parse for XML tool calls
            text_response = ""
            for response_item in model_response:
                if isinstance(response_item, TextResult):
                    text_response += response_item.text

            # Parse XML tool calls from the text response
            xml_tool_calls = self._parse_xml_tool_calls_from_response(text_response)

            if len(xml_tool_calls) == 0:
                # No tools were called, so assume the task is complete
                self.logger_for_agent_logs.info("[no XML tool calls found]")
            ...
            # Process ALL XML tool calls sequentially (like Suna does)
            self.logger_for_agent_logs.info(
                f"Found {len(xml_tool_calls)} XML tool calls, executing all sequentially"
            )

            for tool_call_index, tool_call_dict in enumerate(xml_tool_calls):
                tool_call = self._create_tool_call_parameters(tool_call_dict)
                ...
                self.logger_for_agent_logs.info(
                    f"Executing XML tool call {tool_call_index + 1}/{len(xml_tool_calls)}: {tool_call.tool_name} with parameters: {tool_call.tool_input}\n"
                )
                ...
                # Check if we should stop after each tool call
            ...
            self.logger_for_agent_logs.info(f"Successfully executed all {len(xml_tool_calls)} XML tool calls")
    ...
    def get_tool_start_message(self, tool_input: dict[str, Any]) -> str:
        return f"GeminiSuna agent started with instruction: {tool_input['instruction']}"
    ...
    def cancel(self):
        ...
        self.logger_for_agent_logs.info("GeminiSuna agent cancellation requested")
